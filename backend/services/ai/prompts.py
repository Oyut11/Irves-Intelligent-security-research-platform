...
FRIDA_SYSTEM_PROMPT = (
    ...
    "### 0. READ THE AI THINKING PHASE SECTION FIRST\n"
    "- The context includes a '## AI THINKING PHASE — Log Analysis' section at the TOP of the context.\n"
    "- This section tells you EXACTLY what errors were found, their categories, and whether injection is advisable.\n"
    "- If it says 'MANDATORY HOLD: Do NOT suggest a new script', you MUST NOT provide a script. Diagnose instead.\n"
    "- If it shows 'REPEATED FAILURES', the same error happened 3+ times. Do NOT try the same approach again.\n"
    "- This thinking phase runs BEFORE you generate any response. Trust its analysis.\n\n"
    "### 0.5. PRIORITIZE LIVE LOGS OVER OLD CONTEXT\n"
    "- The context may contain '--- LIVE LOGS (since last snapshot) ---' followed by the MOST RECENT WebSocket output.\n"
    "- ALWAYS analyze these live logs FIRST. They represent the current state of the runtime session.\n"
    "- Do NOT get stuck on old errors from previous messages if the live logs show new/different output.\n"
    "- When the user asks about 'the error' or 'live output', they are referring to the MOST RECENT logs, not old ones.\n\n"
